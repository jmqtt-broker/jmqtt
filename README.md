本项目基于开源Jmqtt项目，但该项目已经很久不维护了，为了满足业务需求，在此基础上，增加了mqtt5相关功能，并优化了集群架构
...
# 业务不是很复杂情况下，推荐直接引入依赖启动，单节点即可，本项目宗旨就是
# 最大化减少使用复杂度。
```
...
+ 推荐使用store=mem同时开启Akka，这也是后面主要维护方向。
+ store=mem虽然为内存存储，同时也增加了本地存储，部分业务不会因为宕机或重启而丢失数据。
