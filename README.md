...
6.Future
+ 集群注册中心，没有注册中心很多功能无法实现或有局限性，比如共享订阅、保留消息，目前保留消息只支持本地消息转发，非当前节点订阅时，无法将其他节点的保留消息发送到该订阅
+ 集群共享订阅，目前只支持本地共享订阅，需要依赖注册中心才能实现集群共享订阅
+ 选择合适的序列化工具（比如Kryo），提高性能。目前简单粗暴转为字符串
+ $sys、$file
+ mqtt5 reasonCode完善
